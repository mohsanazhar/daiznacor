...
@section('title') @lang('translation.corregimiento') @endsection
...
@section('content')
    @component('components.breadcrumb')
        ...
        @slot('title') @lang('translation.corregimiento')  @endslot
    @endcomponent
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex">
                    <h5 class="card-title mb-0">@lang('translation.list') @lang('translation.corregimiento')  </h5>
                        ...
                        <div class='p-1'>
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a id="addCorregimiento" class="dropdown-item" href="javascript:;">@lang('translation.new') </a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    <!-- Ajax Add Modal -->
    <div class="modal fade" id="corregimientoAddModel" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="corregimientoAddModelLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="corregimientoAddModelLabel"></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
...
@section('script')
    ...
    <script>
        ...
        $("#addCorregimiento").on("click", function(){
            var url_var = "<?= url('settings/get-corregimiento-form/'); ?>";
            $.ajax({
                url: url_var ,
                type: 'GET',
                dataType: 'html',
                success: function(data) {
                    $("#corregimientoAddModel").html(' ');
                    $("#corregimientoAddModel").html(data);
                    $("#corregimientoAddModel").modal('show');
                },
        ...
        function editCorregimiento(id){
            var url_var = "<?= url('settings/get-corregimiento-form/'); ?>"+'/'+id;
            $.ajax({
                url: url_var ,
                type: 'GET',
                dataType: 'html',
        ...
        function corregimientoStore (){
            var url_var = "<?= url('settings/storeCorregimientoAjax/'); ?>";
            $.ajaxSetup({
            });
            $.ajax({
                url: url_var,
                method: 'POST',
                data: $('#corregimientoform').serialize(),
                success: function(data){
                    $("#corregimientoAddModel").html(' ');
                    $("#corregimientoAddModel").html(data.view);
        ...
        function corregimientoUpdate(){
            var url_var = "<?= url('settings/updateCorregimientoAjax/'); ?>";
            $.ajaxSetup({
            });
            $.ajax({
                url: url_var,
        ...
        function deleteCorregimiento(id){
            var url_var = "<?= url('settings/deleteCorregimiento/'); ?>"+'/'+id;
            $.ajax({
                url: url_var ,
                type: 'GET',
                dataType: 'html',
