        ...
        <form action="" id="districtform" class="needs-validation" method="POST">
            ...
            <div class="modal-body">
                <div>
                    ...
                    <input type="text" id="district-name" value="{{ $district['name'] }}" class="form-control {{ ($errors->first('district'))?'is-invalid':'' }}" name="district" placeholder="@lang('translation.district')">
                ...
                <div>
                    ...
                    <select {{ (count($provinces) > 0)?'':'disabled' }} id="district-select-edit" name="selectedProvince" class="form-select form-control {{ ($errors->first('selectedProvince'))?'is-invalid':'' }}" aria-label="Default select example">
                        ...
                        @foreach ($provinces as $province)
                            <option value="{{ $province['id'] }}" {{ ($province['id'] == $district['province_id']) ? 'selected' : 'hello' }}>{{ $province['name'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" onclick="districtUpdate()" id="addDistrictSubmit" class="btn btn-primary">@lang('translation.submit')</button>
            </div>
        </form>
    </div>
</div>